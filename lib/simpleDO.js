...
SimpleDO.prototype.find = function(opt, next) {
	var self = this;
	util.deepCopy(opt, this.options)
	// console.log(self.options)
	db.find(self.options, function(err, rs) {
		if(err) return next(err)
		self.recordset = rs
		if(self.relations.length === 0) return next(err,rs)
...
*/
SimpleDO.prototype.makeup = function(data1, data2) {
