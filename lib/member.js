...
exports.audioPaper = function(req, res, next) {
	var opt = {
		...
		limit: {
			...
			end: req.query.end || 10000
		}
	}
	//for already release client, give large range so that it got all data
	
	logger.debug("Request AudioPaper of Member'ID:"+opt.query.creator_id+", From "+opt.limit.start+", Size "+opt.limit.end);
    
  audioPaper.findByMember(opt, function(json) {
		if(json.member.audioPaper.length === 0) {
			return res.send(204);
		} else {
			return res.json(json);
		}
