    ...
    describe('#Sync write records without COMMITED data', function() {
        it('Should success sync WITHOUT any upload write records of '+theMember.username, function(done) {
            ...
            .end(function(res) {
                ...
                var wrs = res.body.writeRecord||[];
                syncWithCommited(sid, theMember, wrs);
                done();
            });
        });
    })
...
function syncWithCommited(sid, theMember, wrs) {
    describe('#Sync write records with COMMITED data', function() {        
        var json = {};
        json.commited = wrs.map(function(wr) { return wr._id; });
        ...
        it('Should success sync WITH all of possible data of '+theMember.username, function(done) {
            ...
            for(var i = 1; i <= 5; i++){
                ...
                json.uncommit.push(record);
            }
            json.commited = json.commited.filter(function(_id, _i, _array) { 
                return (_i%2 === 0);
            });
            member.put('/write_records', json, sid)
            .end(function(res) {
